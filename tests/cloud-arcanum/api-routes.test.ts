...
const CLOUD_ARCANUM_API_ROUTE_TIMEOUT_MS = 15_000;
...
describe("cloud arcanum api routes", () => {
  it("serves cards list and detail responses", async () => {
    ...
    await app.close();
  }, CLOUD_ARCANUM_API_ROUTE_TIMEOUT_MS);
  ...
  it("does not expose Cloud Arena session routes from the Cloud Arcanum API app", async () => {
    const app = await createTestApp();
