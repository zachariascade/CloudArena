...
- Use `quantity-based entries` as the only saved card representation
- Preserve the deck entry order in storage and in battle expansion
- Allow duplicate copies, but only enforce minimum-size validation in Phase 0
- Exclude sideboards from V1
- Limit the deckbuilder to Cloud Arena cards only
- Define deck ids as stable `deck_...` ids generated by the app at save time
- Keep built-in presets distinct from user-saved decks
- Reuse the existing deck metadata fields:
  - `name`
  - `tags`
  - `notes`
...
- the schema is narrow enough that later phases do not need to revisit the basic deck model
...
- saved decks expand deterministically in stored order, repeating each `cardId` by its `quantity`
...
- card search should only return cards legal for Cloud Arena deckbuilding
