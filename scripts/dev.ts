...
function startManagedProcess(processName: "api" | "web", scriptPath: string): ChildProcess {
  ...
  child.on("exit", (code, signal) => {
    ...
    if (processName === "api" && apiChild === child) {
      apiChild = null;
    }
...
async function restartServers(): Promise<void> {
  ...
  await Promise.all([terminateChild(apiChild), terminateChild(webChild)]);
...
function shutdown(exitCode = 0): void {
  ...
  void Promise.all([terminateChild(apiChild), terminateChild(webChild)]).finally(() => {
    setTimeout(() => {
...
if (watchMode) {
  ...
  console.log(
    "Cloud Arcanum local dev is running with TypeScript and frontend bundle watchers. Press Ctrl+C to stop.",
  );
} else {
