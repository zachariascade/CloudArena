...
const ROOT_CATEGORY = "Category:Spirits";
const CATEGORY_FIELD = "Spirit";
const OUTPUT_DIRECTORY = ["exports", "bible-wiki-spirits"];
const OUTPUT_FILENAME = "spirits.csv";
...
const BOOK_ALIASES = new Map<string, string>(
  BOOK_ORDER.flatMap((book) => {
    const variants = new Set<string>([
      ...
      `Book of ${book}`,
      `Spirits in ${book}`,
      `Spirits in the Book of ${book}`,
      `${book} spirits`,
    ]);

    if (GOSPELS.has(book)) {
      variants.add(`Gospel of ${book}`);
      variants.add(`Spirits in the Gospel of ${book}`);
    }
...
function serializeCsv(records: SpiritRecord[]): string {
  const rows = [
    ["simple_name", "where_its_from", "category"].join(","),
    ...records.map((record) =>
      [record.simpleName, record.whereItsFrom, record.category]
        .map(escapeCsvValue)
        .join(","),
    ),
  ];
...
async function crawlSpiritPages(): Promise<Map<number, string>> {
...
function extractBooksFromCategory(categoryName: string): string[] {
  ...
  for (const book of BOOK_ORDER) {
    const patterns = [
      ...
      new RegExp(`Spirits in (the )?(Book of |Gospel of )?${escapeRegExp(book)}`, "i"),
      new RegExp(`${escapeRegExp(book)} spirits`, "i"),
    ];
...
async function main(): Promise<void> {
  ...
  const pages = await crawlSpiritPages();
