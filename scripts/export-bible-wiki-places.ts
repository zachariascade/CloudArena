...
const ROOT_CATEGORY = "Category:Places";
const CATEGORY_FIELD = "Place";
const OUTPUT_DIRECTORY = ["exports", "bible-wiki-places"];
const OUTPUT_FILENAME = "places.csv";
...
const BOOK_ALIASES = new Map<string, string>(
  BOOK_ORDER.flatMap((book) => {
    const variants = new Set<string>([
      ...
      `Book of ${book}`,
      `Places in ${book}`,
      `Places in the Book of ${book}`,
      `${book} places`,
    ]);

    if (GOSPELS.has(book)) {
      variants.add(`Gospel of ${book}`);
      variants.add(`Places in the Gospel of ${book}`);
    }
...
function serializeCsv(records: PlaceRecord[]): string {
  const rows = [
...
async function crawlPlacePages(): Promise<Map<number, string>> {
...
function extractBooksFromCategory(categoryName: string): string[] {
  ...
  for (const book of BOOK_ORDER) {
    const patterns = [
      ...
      new RegExp(`Places in (the )?(Book of |Gospel of )?${escapeRegExp(book)}`, "i"),
      new RegExp(`${escapeRegExp(book)} places`, "i"),
    ];
...
async function main(): Promise<void> {
  ...
  const pages = await crawlPlacePages();
